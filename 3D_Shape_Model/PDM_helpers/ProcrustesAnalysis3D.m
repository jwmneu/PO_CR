...
normX = zeros(size(x));		% shape at the origin, by translation 
normY = zeros(size(y));
normZ = zeros(size(z));
...
for i = 1:size(x,1)
    ...
    % offset translation for every image
    normX(i,:) = x(i,:) - offsetX;
    normY(i,:) = y(i,:) - offsetY;
    normZ(i,:) = z(i,:) - offsetZ;
...
for i = 1:size(x,1)		% for every image
    
    % offset scale for every image
...
for i = 1:30		% iterate 30 times
    ...
    for j = 1:size(x,1)			% for every image
                
        currentShape = [normX(j,:); normY(j,:); normZ(j,:)]';
      
	% we want to align the current shape to the mean one
	% get new rotation matrix and tranlation matrix of current image
        [ R, T ] = AlignShapesKabsch(currentShape, meanShape);		
	
	% update eulers parameters of rotation
        eulers = Rot2Euler(R);
	...
	% offset the rotation effect of current image to get a better mean shape
...
%     end
	...
	% offset scale for every image to get better mean shape
    ...
    if(diff/norm(oldMean,'fro') < change)		% less than 30 iterations
        break;
    end
