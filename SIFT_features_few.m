...
function [d] = SIFT_features_few(input_image, lm, SIFT_scale, num_of_lm)
    load shape_model;
    load myShape;
    load myAppearance;
    load fd_stat;
...
%     face_size = (max(lm(:,1)) - min(lm(:,1)) + max(lm(:,2)) - min(lm(:,2)))/2;
    ...
    fc = [ lm'; ones(1, num_of_lm) * SIFT_scale; ones(1, num_of_lm) * (-pi/8)];   % scale of SIFT is determined by face_size. rotation is unknown. 
