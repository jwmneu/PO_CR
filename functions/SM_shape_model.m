...
%% test correctness of shape parameters
	for gg = 1 : n
		[pt_pt_err_image(gg)] = Compute_pt_pt_error_image1([p_rigid(gg, :), p_nonrigid(gg, :)], TR_gt_landmarks{gg}, lm_ind1);
	end
	var = 0 : 0.01 : 0.07; 
	[pt_pt_err_allimages, cum_err] = Compute_cum_error(pt_pt_err_image, var, n, 'cum error of shape model', 1);
...
function [lmrect] = reconstruct_lm(p_nonrigid, p_rigid, lm_ind1)
	modelDir = 'matfiles/';
...
function [pt_pt_err_image] = Compute_pt_pt_error_image1(p_mat_gg, gt_landmark, lm_ind1)
	lm = reconstruct_lm(reshape(p_mat_gg(1,5:end), 1, []), reshape(p_mat_gg(1,1:4), 1, []));
	pt_pt_err_image = compute_error(gt_landmark(lm_ind1,:), lm );
end
