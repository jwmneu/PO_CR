function [p_mat_rigid_initialization_n, p_mat_nonrigid_initialization_n, Perturbed_SIFT_Feature_labels_n] = Collect_Initial_Perturbations(n1, n2, SIFT_scale, Kpi)
	...
	if SAVE == 1
		save([saveDir 'Perturbed_SIFT_Feature_labels_' num2str(n1) '-' num2str(n2) '_S-' num2str(SIFT_scale) '_Kpi-' num2str(Kpi) '.mat'], 'Perturbed_SIFT_Feature_labels_n');
		save([saveDir 'p_mat_rigid_initialization_' num2str(n1) '-' num2str(n2) '_S-' num2str(SIFT_scale) '_Kpi-' num2str(Kpi) '.mat'], 'p_mat_rigid_initialization_n');
		save([saveDir 'p_mat_nonrigid_initialization_' num2str(n1) '-' num2str(n2) '_S-' num2str(SIFT_scale) '_Kpi-' num2str(Kpi) '.mat'], 'p_mat_nonrigid_initialization_n');
	end
end