...
%    	[TR_images, TR_face_size, TR_gt_landmarks, TR_myShape_3D_p, TR_detections] = Collect_training_images_3D(n1, n2) ;
	...
	for gg = 1:n
		...
		for k = 1 : Kpi
			...
			% construct noise
			noise = zeros(1, K);
			noise(1, 1) =  fd_stat_SM_3D.mean_delta_scale(1,1) + noise_scale * fd_stat_SM_3D.std_delta_scale(1,1) * randn(1); 
			noise(1, 2:3) = (fd_stat_SM_3D.mean_delta_translation(1, :) + fd_stat_SM_3D.std_delta_translation(1, :) .* randn(1,2)) * face_size; 
			noise(1, 4:6) =  fd_stat_SM_3D.std_rotation_euler(1,:) .* randn(1, 3);   %fd_stat_SM_3D.mean_rotation_euler(1,:) +
