function [] = Split_training_validation_set(n, SIFT_scale, Kpi, saveDir)
	...
	% It collect ground truth parameters from TR_params and TR_3D_params folder, split
	% into training and validation set, and save result to saveDir = '../Learning_Inputs/'. 
	
	if nargin == 0
		n = 2811; 
		SIFT_scale = 2; 
		Kpi = 10;
		saveDir = '../Learning_Inputs/';
	end
	...
	split_gt_params(gtParamDir, indexes_train, indexes_validate, n_train, n_validate, n, saveDir); 
 	
% 	% split perturbation initializations and save
% 	InitializationDir = ['../PertInit_training_LM_3D_0.35_all_params/'];
% 	split_perturbation_initialization(InitializationDir, n_train, n_validate, indexes_train, indexes_validate, list_train, list_validate, Kpi, ...
...
function [] = split_gt_params(gtParamDir, indexes_train, indexes_validate, n_train, n_validate, n, saveDir)	
	load([gtParamDir 'TR_training.mat']); 
	TR_images_train = {};
	TR_images_validate = {};
	if(exist([gtParamDir 'TR_training_images.mat'], 'file') == 2)
		load([gtParamDir 'TR_training_images.mat']);
		TR_images_train = TR_images(indexes_train); 
	...
	save([saveDir 'TR_images_train.mat'], 'TR_images_train', '-v7.3');
	save([saveDir 'TR_images_validate.mat'], 'TR_images_validate', '-v7.3');
	save([saveDir 'TR_Training_Params.mat'], 'TR_detections_train', 'TR_face_size_train', 'TR_gt_landmarks_train', 'TR_myShape_3D_p_train', 'n_train', 'n'); 
	save([saveDir 'TR_Validation_Params.mat'], 'TR_detections_validate', 'TR_face_size_validate', 'TR_gt_landmarks_validate', 'TR_myShape_3D_p_validate', 'n_validate', 'n'); 
	save([saveDir 'TR_Training_Validation_Params.mat'], 'TR_detections_train', 'TR_face_size_train', 'TR_gt_landmarks_train', ...
		'TR_myShape_3D_p_train', 'TR_detections_validate', 'TR_face_size_validate', 'TR_gt_landmarks_validate', 'TR_myShape_3D_p_validate'); 
end

function [] = split_perturbation_initialization(InitializationDir, n_train, n_validate, indexes_train, indexes_validate, list_train, list_validate, Kpi, ...
			SIFT_scale, plot, saveDir, this_version_check)
	...
	load([InitializationDir 'Perturbed_Shape_Param_Initialization_LM_3D.mat']);
	...
	save([saveDir 'PertInit_Training_Validation_Params.mat'], 'delta_p_initialization_train', 'p_mat_initialization_train', 'features_train', 'b_mat_train', ...
		...
		'p_mat_gt_validate', 'pt_pt_err0_image_validate', 'n_train', 'n_validate', 'indexes_train', 'indexes_validate', 'list_train', 'list_validate', 'SIFT_scale', 'Kpi');
end
